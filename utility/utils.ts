...
class Utils {
  ...
  async createData(model: any, data: any) {
    ...
    try {
      ...
      let returnObject = this.returnObj(
        [createData],
        statusCodes.success,
        modelName,
        "create"
      ...
      logger.info(
        `${this.fName} : Successfully created record for : ${modelName}`
      );
      return returnObject;
    } catch (error) {
  ...
  async updateData(model: any, data: any, incidentId:number) {
    let modelName = model.name;
    try {
      const getModelID = await appDataSource.getRepository(model).getId(data);
      logger.info(modelName, getModelID, model)
      ...
      const updateData = await appDataSource
        ...
        .update(Incident)
        .set(data)
        .where("incident.incidentId = :id", {id: 10})
        .execute();
      ...
      logger.info(
        `${this.fName} : Successfully updated record for : ${modelName}`
      );
      return returnObject;
    } catch (error) {
  ...
  async getAllData(model: any, data: any) {
    ...
    try {
      ...
      logger.info(
        `${this.fName} : Successful Retrival record(s) for : ${modelName}`
      );
      return returnObject;
    } catch (error) {
      logger.info(
        `${this.fName} : Error retriving record(s)for : ${modelName}`
      );
      logger.info(error);
      let returnObject = this.returnObj(
        [error],
  ...
  async getbyIDData(model: any, data: any) {
    let modelName = model.name;
    try {
      const createData = await appDataSource
        .getRepository(model)
        .findOneBy(data);
      let returnObject = this.returnObj(
        [createData],
      ...
      logger.info(
        `${this.fName} : Successful retrival record for : ${modelName}`
      );
      return returnObject;
    } catch (error) {
  ...
  async saveData(model: any, data: any) {
    ...
    try {
      ...
      logger.info(
        `${this.fName} : Successfully saved record for : ${modelName}`
      );
      return returnObject;
    } catch (error) {
  ...
  async deleteData(model: any, data: any) {
    ...
    try {
      ...
      logger.info(
        `${this.fName} : Successfully deleted record for : ${modelName}`
      );
      return returnObject;
    } catch (error) {
      ...
      for await (const data of csvData) {
        ...
        returnData.depts =
          data.department.trim().toUpperCase() == ""
            ? returnData.depts + ""
            : returnData.depts.search(data.department.trim().toUpperCase()) ==
              -1
            ? returnData.depts + data.department.trim().toUpperCase() + ","
            : returnData.depts + "";
        const employeeData = await Util.getbyIDData(Employee, {
